...
store.push({userName: masterUser, caffeineLevel: initialOfficeLevels,
	caffeineLong: initialOfficeLevelsLong, caffeineMax: 0, caffeineLongMax: 0, caffeineCurrent: 0, caffeineLongCurrent: 0,
	caffeineLevelObject: {}, caffeineLongObject: {}});
...
app.post("/api/user/add", function (req, res) {
	...
	if ((store.find(store => store.userName === req.body.userName.toUpperCase()) === undefined)
		&& store.length<maxLimit && req.body.userName.length<maxLimit) {
		let initialLevels = new Array(maxLimit).fill(0);
		let initialLevelsLong = new Array(maxLimit).fill(0);
		store.push({userName: req.body.userName.toUpperCase(), caffeineLevel: initialLevels, caffeineLong: initialLevelsLong,
		caffeineMax: 0, caffeineLongMax: 0, caffeineCurrent: 0, caffeineLongCurrent: 0,
		caffeineLevelObject: {}, caffeineLongObject: {}});
		store = store
            .filter(e => e.userName === masterUser)
            .concat(
           		store.filter(e => e.userName !== masterUser)
...
app.post("/api/drink/add", function (req, res) {
	...
	store.filter(function( obj ) {
    	...
    	if (obj.userName === req.body.userName && req.body.userName !== masterUser
    		&& !isNaN(caffeineNumber) && caffeineNumber>0 && caffeineNumber<maxLimit
    		&& obj.caffeineLevel[obj.caffeineLevel.length-1]<maxLimit) {
    		obj.caffeineLevel[obj.caffeineLevel.length-1] =
    		Number(obj.caffeineLevel[obj.caffeineLevel.length-1])+ Number(caffeineNumber);
    	}
	});
	let officeCaffeineLevel = 0;
	store.forEach(function (userData) {
...
console.log(`${Date()}\nEskoffee Caffeine Decision Support started in port: ${process.env.server_port}`);
